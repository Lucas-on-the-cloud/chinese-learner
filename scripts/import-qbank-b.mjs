// Import Band B Question Bank → Supabase TOCFL tables (band='QB-B').
// Parses TXT files + Answer XLSX, uploads MP3s to R2, upserts to DB.
//
// Usage:
//   node scripts/import-qbank-b.mjs --dryrun          # parse only, print counts
//   node scripts/import-qbank-b.mjs --skip-upload      # no R2 upload, still writes DB
//   node scripts/import-qbank-b.mjs                    # full run
...
const QB   = path.join(ROOT, 'question_bank', 'bandB');
...
function r2url(r2Key) { return `${R2_PUBLIC_URL}/${r2Key}`; }
...
function parseAnswerXlsx(filePath, hasAudio) {
  ...
  const rows = xlsx.utils.sheet_to_json(sh, { header: 1 }).slice(1);
...
function parseListeningScript(content, numQuestions) {
  ...
  return { sharedText: sharedLines.join('\n').trim(), questionTexts };
}
...
function parseGapFilling(content) {
  ...
  for (const line of optLines) {
    ...
    for (let i = 0; i < allOpts.length && i < currentNums.length; i++) {
      questions[currentNums[i]].options[allOpts[i][1]] = allOpts[i][2].trim();
    }
  }
  return { passageText, questions: Object.values(questions).sort((a, b) => a.qNum - b.qNum) };
}
...
function parseReadingComp(content) {
  ...
  return { passageText: passageLines.join('\n').trim(), questions: Object.values(questions).sort((a, b) => a.qNum - b.qNum) };
}
...
// ─── Main ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log(`\n═══ Import Band B Question Bank${DRYRUN ? ' [DRYRUN]' : ''}${SKIP_UPLOAD ? ' [SKIP_UPLOAD]' : ''} ═══\n`);

  const exams = [], parts = [], groups = [], questions = [], options = [];
  let totalMp3Uploads = 0, totalMp3Skipped = 0;

  // ── ID ranges (safe above QB-C: exam=1000, parts=2001-2004, groups=5001-5200, q=10001-10600, opt=50001-51800) ──
  const EXAM_ID = 1001;
  let partId    = 3000;
  let groupId   = 6000;
  let qId       = 12000;
  ...
  async function maybeUploadMp3(localMp3Dir, audioId, r2Subdir) {
    ...
    const r2Key   = `qbank/bandb/${r2Subdir}/${fname}`;
  ...
  console.log('── Dialogue ──');
  {
    ...
    const xlsxGroups   = parseAnswerXlsx(path.join(QB, 'B_L_Dialogue_Answer.xlsx'), true);
    const tDir   = path.join(QB, 'B_L_Dialogue_T (1)');
    const lsDir  = path.join(QB, 'B_L_Dialogue_Listening Script');
    const mp3Dir = path.join(QB, 'B_L_Dialogue_MP3');
    ...
    for (const xg of xlsxGroups) {
      ...
      for (const xq of xqs) {
        ...
        questions.push({
          ...
          question_image_url: null, explanation: null,
        });
  ...
  console.log('── Monologue ──');
  {
    ...
    const qCountBefore = questions.length;
    const xlsxGroups   = parseAnswerXlsx(path.join(QB, 'B_L_Monologue_Answer.xlsx'), true);
    const tDir   = path.join(QB, 'B_L_Monologue_T');
    const lsDir  = path.join(QB, 'B_L_Monologue_Listening Script');
    const mp3Dir = path.join(QB, 'B_L_Monologue_MP3');
  ...
  console.log('── Gap Filling ──');
  {
    ...
    const qCountBefore = questions.length;
    const xlsxGroups   = parseAnswerXlsx(path.join(QB, 'B_R_Gap Filling_Answer.xlsx'), false);
    const tDir = path.join(QB, 'B_R_Gap Filling_T', 'B_R_Gap Filling_T');
    ...
    for (const xg of xlsxGroups) {
      ...
      const { passageText, questions: parsed } = parseGapFilling(fs.readFileSync(tFile, 'utf8'));
  ...
  console.log('── Reading Comprehension ──');
  {
    ...
    const qCountBefore = questions.length;
    const xlsxGroups   = parseAnswerXlsx(path.join(QB, 'B_R_Reading Comprehension_Answer.xlsx'), false);
    const tDir = path.join(QB, 'B_R_Reading Comprehension_T', 'B_R_Reading Comprehension_T');
